    ...
    case Some(config) =>
      val genePatientMatrix = {
        ...
        val mutationMatrix = if (config.maf.isDefined) {
          ...
          helper.loadMafDataWithFunctionalScore(config.maf.get, maxQtyMutations = config.maxQtyMutations)
        } else {
      ...
      helper.printMutationMatrixFiles(config.output, config.seedGenesMutations, genePatientMatrix, true, config.outputFolder)
