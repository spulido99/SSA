...
object MutualExclusivitySimulations extends App {
  ...
  val parser = new scopt.OptionParser[InputConfig]("SSA.ME.") {
    opt[String]('b', "input-folder") required () action { (x, c) =>
      c.copy(inputFolder = x)
    } text ("Path to the input folder where the reference network is situated.")
  }
   ...
   */
      ...
      */
       ...
       */
          val genePatientMatrix: Map[PolymorphismKey, Polymorphism] = buildRandomGenePatientMatrix ++ buildMEGenePatientMatrix(causalNetwork)
          /*
        ...
        for (rep <- reps) {
          ...
          for (r <- rs) {
            for (f <- fs) {
              ...
              if (development) {
                PrintFiles.printPattern("sim", rankedGenes, networkManager, genePatientMatrix, "", "./src/main/resources/cancergenelists")
              }
   ...
   */
      ...
      def runSimulation(genePatientMatrix: Map[PolymorphismKey, Polymorphism], causalNetwork: Network, noise: Double, r: Int, writer: FileWriter, noisyNetwork: Network, geneList: Set[Gene], subnetworkSize: Option[Int] = None) = {
        println("Number of Walkers: " + geneList.size)
    ...
    ("TCGA-A2-A1G6",  7))*/
        ...
        val meGenePatientMatrix = new HashMap[PolymorphismKey, Polymorphism]
        ...
        while (!samplesToAssign.isEmpty) {
          ...
          if (random.nextDouble() < 1.0 / math.min((index + 1.0) * 3, 30.0)) {
            ...
            if (true) { //}!noMutableMEGenes.contains(gene)) {
              ...
              meGenePatientMatrix.put(PolymorphismKey(gene, sample, 0), Polymorphism(gene.name))
           ...
           */
              // Location not yet implemented here !
              meGenes.filter { g => random.nextDouble() < 0.2 / subnetworkSize }.foreach { g => meGenePatientMatrix.put(PolymorphismKey(g, sample, 0), Polymorphism(g.name)) }
          ...
          (0 until m._2).map { i =>
            {
              ...
              // Location not yet implemented here !
              val key = PolymorphismKey(randomGene, m._1, 0)
              val value = Polymorphism(randomGene.name)
              (key, value)
            }
          }
        }
