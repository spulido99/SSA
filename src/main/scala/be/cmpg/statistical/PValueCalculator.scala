...
import be.cmpg.cancer.GeneAnnotation
...
object PValueCalculator extends App {
  ...
  val parser = ArgumentsParser.getBasicArgParser("SSA.ME P-Value calculation")
  ...
  if (numberOfLines < config.pvalueExperiments) {
    ...
    val alterationsBySample = if (config.pvaluetype == "bootstraap") genePatientMatrix.toList.groupBy(_._1.sample) else Map[String, List[(PolymorphismKey, Polymorphism)]]()
    ...
    for (e <- numberOfLines to config.pvalueExperiments) {
      ...
      val randomizedGenePatientMatrix = if (config.pvaluetype == "distance" || config.pvaluetype == "closegenes") {
          DataRandomizations.getCloseGenesRandomizedGenePatientMatrix(genePatientMatrix, annotationMap)
        } else if (config.pvaluetype == "bipartite") {
      	  DataRandomizations.randomizeBipartiteGenePatientMatrix(genePatientMatrix, all_samples, altered_genes intersect network.genes)
        } else if (config.pvaluetype == "genenames") {
          DataRandomizations.randomizeGeneNames(genePatientMatrix, altered_genes intersect network.genes)
        } else if (config.pvaluetype == "bootstraap") {
          DataRandomizations.bootstrapSamples(alterationsBySample)
        } else {
          throw new RuntimeException("p-value type not known: "+config.pvaluetype)
        }
      ...
      val walkers = WalkerFactory.buildHi2iiFungusWalker(geneList.view.toSet, pvalNetworkManager)
